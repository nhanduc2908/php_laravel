<template>
    <div class="users-management"><div class="header-actions"><h2>Quản lý người dùng</h2><button class="btn-primary" @click="openCreateModal"><i class="fas fa-plus"></i> Thêm người dùng</button></div>
    ...
    <Pagination :current-page="currentPage" :total-pages="totalPages" @page-change="fetchUsers" /></div>
</template>
...
const userStore = useUserStore(); const users = ref([]); const search = ref(''); const roleFilter = ref(''); const currentPage = ref(1); const totalPages = ref(1)
...
const editUser = (user) => { /* Edit logic */ }; const deleteUser = async (id) => { if (confirm('Xóa?')) await userStore.delete(id); fetchUsers() }; const resetPassword = async (id) => { await userStore.resetPassword(id); alert('Đã gửi email reset mật khẩu') }
