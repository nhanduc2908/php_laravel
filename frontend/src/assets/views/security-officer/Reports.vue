<template>
    ...
    <tbody><tr v-for="r in reports" :key="r.id"><td>{{ r.name }}</td><td>{{ r.server_name }}</td><td>{{ r.format }}</td><td>{{ formatDate(r.created_at) }}</td>
    <td class="actions"><button @click="downloadReport(r.id)"><i class="fas fa-download"></i></button></td></tr></tbody>
